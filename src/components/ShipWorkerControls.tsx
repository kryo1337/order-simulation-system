...
interface ShipWorkerControlsProps {
  onProcessed?: () => void;
}
...
export default function ShipWorkerControls({ onProcessed }: ShipWorkerControlsProps) {
  ...
  const processSingleOrder = useCallback(async (): Promise<boolean> => {
    try {
      const response = await fetch(
        `/api/workers/ship?simulate=true`,
        { method: 'POST' }
      );
  ...
  const handleProcessOne = useCallback(async () => {
    ...
    setError(null);
    setCurrentStatus('Wysyłanie zamówienia...');
    ...
    if (!hasMore) {
      ...
      setCurrentStatus('Zamówienie wysłane');
    }
    ...
    <div className="bg-gray-800 rounded-lg p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="flex-1 py-3 px-4 rounded-lg font-medium bg-emerald-600 hover:bg-emerald-700 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isProcessing && !isPolling ? 'Wysyłanie...' : 'Wyślij Jedno'}
            </button>
          </div>
      ...
      <div className="border-t border-gray-700 pt-6">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-gray-900 rounded-lg p-4">
            <div className={`text-3xl font-bold ${isPolling ? 'text-green-400' : 'text-emerald-400'}`}>
