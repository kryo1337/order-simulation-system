...
interface InvoiceWorkerControlsProps {
  onProcessed?: () => void;
}
...
export default function InvoiceWorkerControls({ onProcessed }: InvoiceWorkerControlsProps) {
  ...
  const processSingleOrder = useCallback(async (): Promise<boolean> => {
    try {
      const response = await fetch(
        `/api/workers/invoice?simulate=true`,
        { method: 'POST' }
      );
  ...
  const handleProcessOne = useCallback(async () => {
    ...
    setError(null);
    setCurrentStatus('Wystawianie faktury...');
    ...
    if (!hasMore) {
      ...
      setCurrentStatus('Faktura wystawiona');
    }
    ...
    <div className="bg-gray-800 rounded-lg p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="flex-1 py-3 px-4 rounded-lg font-medium bg-amber-600 hover:bg-amber-700 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isProcessing && !isPolling ? 'Wystawianie...' : 'Wystaw Jedną'}
            </button>
          </div>
          ...
          {currentStatus && (
            <div className="text-sm text-gray-400 text-center">
              {isProcessing && (
                <span className="inline-block w-2 h-2 bg-orange-500 rounded-full animate-pulse mr-2" />
              )}
              {currentStatus}
            </div>
          )}
      ...
      <div className="border-t border-gray-700 pt-6">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-gray-900 rounded-lg p-4">
            <div className={`text-3xl font-bold ${isPolling ? 'text-green-400' : 'text-amber-400'}`}>
